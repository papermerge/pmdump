...
func GetInboxFlatNodes(db *sql.DB, user_id int) ([]models2.FlatNode, error) {
  ...
  `
	...
	var nodes []models2.FlatNode
	var discard int

	for rows.Next() {
		var node models2.FlatNode
		err = rows.Scan(
			&node.ID,
			&node.Title,
			&node.Model,
...
func GetHomeFlatNodes(db *sql.DB, user_id int) ([]models2.FlatNode, error) {
...
func GetUserNodes(db *sql.DB, user *models2.User) error {

	user.Inbox = &models2.Node{
		Title:    "inbox",
		NodeType: models2.FolderType,
	}
	user.Home = &models2.Node{
		Title:    "home",
		NodeType: models2.FolderType,
	}
